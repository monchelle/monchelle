...
function boards(): Board[] {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((x) => x?.company && x?.token)
      : [];
  } catch {
    throw new Error(
      "GREENHOUSE_BOARD_TOKENS_JSON must be valid JSON."
    );
  }
}
...
function score(profile: any, job: any) {
  ...
  const titleHits = titleWords.filter(
    (x: string) => resume.includes(x)
  ).length;
  ...
  if (
    ...
    (profile.preferred_work_arrangements || [])
      .join(" ")
      .toLowerCase()
      .includes("remote")
  ) {
...
async function fetchBoard(
  ...
  const url =
    ...
    `${encodeURIComponent(board.token)}/jobs?content=true`;

  const response = await fetch(url);

  if (!response.ok) {
    throw new Error(
      `${board.company}: Greenhouse returned ${response.status}`
    );
  }
...
Deno.serve(async (request) => {
  ...
  try {
            ...
            .upsert(
              {
                ...
                match_explanation:
                  `Greenhouse posting matched against ${
                    profile.full_name ||
                    "your"
                  } saved profile.`,
                calculated_at:
    ...
    const stale = (active.data || [])
      .filter(
        (x) => !seen.has(x.source_job_id)
      )
      .map((x) => x.id);
